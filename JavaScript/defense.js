	...
	checkHealth(index) {
		if(this.health <= 0) {
			...
			if(waveNum < 10) {
				...
				else if(this.type === 'redBarb') playerMoney += 20;
				else if(this.type === 'blueBarb') playerMoney += 20
				else if(this.type === 'greenBarb') playerMoney += 20
				else if(this.type === 'barbArcher1') playerMoney += 25;				
				else if(this.type === 'fastBarb') playerMoney += 25;
				else if(this.type === 'barbArcher2' || this.type === 'skeleton' || this.type === 'fastBarb2') playerMoney += 15;
				else if(this.type === 'barbArcher3') playerMoney += 30;
			}
			else {
				if(this.type === 'easy') playerMoney += 3;
				else if(this.type === 'medium') playerMoney += 8;
				else if(this.type === 'redBarb') playerMoney += 10;
		...
		for(let i = 0; i < enemyArray.length; i++) {
			...
			if(enemySpawnCounter === enemy.time) {
				...
				if(enemy.type === 1) {
					...
					opponent = new Enemy('greenBarb', (.05 * w) - w/60, 1.05 * h, 10, 1.25025, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
				else if(enemy.type === 6) {
					opponent = new Enemy('barbArcher1', (.05 * w) - w/60, 1.05 * h, 12, 1.25025, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
				else if(enemy.type === 7) {
					opponent = new Enemy('fastBarb', (.05 * w) - w/60, 1.05 * h, 9, 3, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
				else if(enemy.type === 8) {
					opponent = new Enemy('fastBarb2', (.05 * w) - w/60, 1.05 * h, 20, 3, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
				else if(enemy.type === 9) {
					opponent = new Enemy('barbArcher2', (.05 * w) - w/60, 1.05 * h, 30, 2.5, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
				else if(enemy.type === 10) {
					opponent = new Enemy('barbArcher3', (.05 * w) - w/60, 1.05 * h, 50, 3, 0, 0, 1, w/30, w/30, enemy.stealth);
				}
