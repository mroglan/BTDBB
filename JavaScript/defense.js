...
function loadEvents() {
	...
	event5 = function(e) {
		if(mousePos.x > .8 * w) {
			...
			else if(mousePos.x > .8 * w && mousePos.x < .825 * w && mousePos.y > .36 * h && mousePos.y < .36 * h + .04 * w && playerMoney >= riflemanCost && waveNum > 26) {
				itemNumSelected = 8;
				addEventListener('mousemove', event6);
				addEventListener('mouseup', event7);
			}
	...
	event10 = function(e) {
		if(mousePos.x > .75 * w) {
			...
			else if(mousePos.x > .8 * w && mousePos.x < .825 * w && mousePos.y > .36 * h && mousePos.y < .36 * h + .04 * w && waveNum > 26) {
				hoverText = 'Rifleman';
				return;
			}
