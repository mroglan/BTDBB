...
let image1, image2, image3, image4, image5, image6, image7, image8, image9, image10, image11, image12;
let event1, event2, event3, event4, event5, event6, event7, event8, event9, event10, event11;
let mousePos;
let endPointsArray = []; 
...
class General {
	...
	registerUpgrades(troop) {
		...
		if(this.levelB === 3) {
			troop.responseTime -= 20;
		}
		else if(this.levelB === 4) {
			troop.responseTime -= 50;
		}
	}
	
	registerUpgradeB(level) {
		this.supportedTroops.forEach(function(troop) {
			if(level === 3) troop.responseTime -= 20;
			else if(level === 4) troop.responseTime -= 30;
